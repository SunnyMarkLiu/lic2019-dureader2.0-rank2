...
from layers.rnet_modules.layers import VariationalDropout, MultiHeadAttention
...
class RnetMatchLayer(object):
    ...
    def match(self, encoder_context, encoder_question, context_len, question_len):
        with tf.variable_scope('rnet'):
            ...
            # co-attention
            co_attention_context = tf.expand_dims(tf.keras.layers.Dense(self.hidden_size)(encoder_context),
                                                  2)  # B*CL*1*H
            co_attention_question = tf.expand_dims(tf.keras.layers.Dense(self.hidden_size)(encoder_question),
                                                   1)  # B*1*QL*H
            ...
            co_attention_output = dropout(co_attention_rnn(co_attention_rnn_input, context_len)[0],
            ...
            self_attention_output = dropout(self_attention_rnn(self_attention_rnn_input, context_len)[0],
                                            self.training)  # B*CL*(H*2)
...
class AttentionFlowMultiHeadMatchLayer(object):
    ...
    def __init__(self, hidden_size, heads, training, dropout_keep_prob=1.0):
        self.hidden_size = hidden_size
    ...
    def match(self, passage_encodes, question_encodes, context_len, question_len):
        with tf.variable_scope('bidaf-multi-head'):
            ...
            question2context_attn = tf.tile(tf.matmul(b, passage_encodes),
                                            [1, tf.shape(passage_encodes)[1], 1])
            bidaf_match_outs = tf.concat([passage_encodes, context2question_attn,
                                          passage_encodes * context2question_attn,
                                          passage_encodes * question2context_attn], -1)
            bidaf_out_hiddensize = bidaf_match_outs.get_shape().as_list()[-1]

            print('bidaf_out_hiddensize:', bidaf_out_hiddensize)
            # multi-head self-attention encoder
            # 注意 hidden_size 能被 heads 整除
            self_attention_encoder = MultiHeadSelfAttentionEncoder(heads=self.heads,
                                                                   input_hidden_size=bidaf_out_hiddensize,
                                                                   out_rnn_hidden_size=self.hidden_size,
                                                                   training=self.training,
                                                                   keep_prob=self.dropout_keep_prob)
            self_attention_output = self_attention_encoder(bidaf_match_outs, context_len)
            return self_attention_output, None
