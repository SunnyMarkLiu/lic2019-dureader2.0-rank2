...
## prepare
```bash
nohup python run.py --prepare --create_vocab true --max_a_len 400 --train_answer_len_cut_bins -1 \
                    > full_data_prepare.log 2>&1 &
```

## train
```bash
nohup python run.py --train --gpu 0 --desc 'full_data_v5' --algo RNET \
...
## evaluate
```bash
python run.py --evaluate --gpu 0 --desc 'full_data_v5' --max_a_len 400 --batch_size 128
```
